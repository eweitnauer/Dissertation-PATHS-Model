...
load_data_old = function() {
  ...
  data = rename(data, c("found_solution"="solved","cond"="pres_mode","train_time"="steps","mturk_id"="rep"));
...
load_data = function(use_all=FALSE,filename="data-0-7-0.csv",pbpList=NULL) {
  ...
  data = rename(data, c("solved"="found_solution","pres_mode"="cond","steps"="train_time","rep"="mturk_id","activity..feature..hyp_base"="feat_base","activity..obj..hyp_base"="obj_base", "activity..selector..hyp_base"="exploit"));
  data$found_solution = ifelse((data$found_solution == 'true') | (data$found_solution == 1), 1, 0);
